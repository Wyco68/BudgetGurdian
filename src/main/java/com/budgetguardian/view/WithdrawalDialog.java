...
/**
 * Modal add/edit dialog for a withdrawal: account, amount, an optional
 * reason and a date — no category, no item name.
 */
public final class WithdrawalDialog {

    private final DataStore store;
    private final Supplier<LocalDate> today;

    public WithdrawalDialog(DataStore store, Supplier<LocalDate> today) {
        this.store = store;
        this.today = today;
    }
    ...
    private Optional<Transaction> show(Transaction existing, Long prefillSatang, String preferredAccountId) {
        ...
        dialog.setTitle(existing == null ? "New Withdrawal" : "Edit Withdrawal");
        ButtonType saveType = new ButtonType(existing == null ? "Add" : "Save", ButtonBar.ButtonData.OK_DONE);
        dialog.getDialogPane().getButtonTypes().addAll(saveType, ButtonType.CANCEL);
        DialogSupport.applyStylesheet(dialog, WithdrawalDialog.class);
    ...
    private Transaction build(Transaction existing, Account account, String amountText,
        ...
        return new Transaction(id, TransactionType.WITHDRAWAL, account.id(), null, null,
